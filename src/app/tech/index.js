...
//生命周期函数，组件加载完成时候立刻调用
  ...
  doSave=()=>{
    //Ant Design是阿里蚂蚁金服团队基于React开发的ui组件
    this.props.form.validateFields(async (err, values) => {
      if (err) { return }
      // console.log(values);//values存放课程基本信息
      values.web = (values.web)?1:0
      // console.log(this.state);
      // tecList是教学进度，expList是实验进度
      let {tecList,code,expList } = this.state
      let params = { code:code,tecList:tecList,expList:expList, ...values }
      debug(params)//实验console.log输出一些东西，不用管，可以注释
      this.setState({ loading: true })//转轮开始转直到加载完成
      // urls.API_SAV_CLS是 http://localhost/savCls
  ...
  render() {
    ...
    return (
      //  React组件通过spin.js展示一个转轮直到你的组件加载完成
      //spinning true的时候有转轮，false的时候没有
            ...
            <div className="m-tab_list">
              ...
              <>
                ...
                <div className="m-fun">
                  ...
                  <Tooltip placement="right" title="导出docx">
                    <div className="m-item" style={{'background':'#b8d800','color':'#fff'}} onClick={this.doExport}>导出docx</div>
                  </Tooltip>
              ...
              <Form className="m-form" layout="horizontal" >
                <div className="m-hd">
                  ...
                  <div className="m-info">
                    <span>{cls?.cform}</span>
                    <span>{cls?.cprop}</span>
                    {/* checkedChildren代表选中的时候显示网，未选中显示普，一个是网课，一个是普通课 */}
                    {/* web是输入控件的唯一标志  */}
                    {/* valuePropName子节点的值的属性
                      initialValue子节点的初始值
                    */}
                    <Form.Item>
